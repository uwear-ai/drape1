...
class BasicTransformerBlock(nn.Module):
    ...
    def forward(self, x, encoder_hidden_states=None, hidden_states_ref=None, mask=None):
        ...
        if hidden_states_ref is not None:
            x_ref = self.attn1(x_norm, hidden_states_ref.to(dtype=x_norm.dtype))
...
class Transformer2DModel_end(nn.Module):
    def __init__(self, in_channels, out_channels, n_layers):
        super(Transformer2DModel_end, self).__init__()
        self.norm = nn.GroupNorm(32, in_channels, eps=1e-06, affine=True)
        self.proj_in = nn.Linear(in_channels, out_channels, bias=True)
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(out_channels) for _ in range(n_layers - 1)]
        )
    ...
    def forward(
        ...
        out_hidden_states_refs.append(hidden_states)
...
class CrossAttnUpBlock2D_end(nn.Module):
    def __init__(self, in_channels, out_channels, prev_output_channel, n_layers):
        super(CrossAttnUpBlock2D_end, self).__init__()
        self.attentions = nn.ModuleList(
            [
                Transformer2DModel(out_channels, out_channels, n_layers),
                Transformer2DModel(out_channels, out_channels, n_layers),
                Transformer2DModel_end(out_channels, out_channels, n_layers),
            ]
        )
        self.resnets = nn.ModuleList(
            [
    ...
    def forward(
        ...
        **kwargs
        ...
        sample, [s4, s5, s6], out_hidden_states_ref = self.down_blocks[1](
            ...
            hidden_states_ref=(
                None if hidden_states_ref is None else hidden_states_ref["down_1"]
            ),
            mask=mask,
        )
        ...
        sample, [s7, s8], out_hidden_states_ref = self.down_blocks[2](
            ...
            additional_residuals=down_add_res,
            hidden_states_ref=(
                None if hidden_states_ref is None else hidden_states_ref["down_2"]
            ),
            mask=mask,
        )
        ...
        return out_hidden_states_refs


class UwearUNet2DConditionModelRef(
    UwearUNet2DConditionModelBase,
    UwearLoaderMixin,
):
